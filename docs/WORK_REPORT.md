...
## 2026-09-11：真实视频性能与失败诊断

本次已经直接访问用户服务器，使用已有模型、EPIC 视频和
`supermemory-pilot-v2` 运行真实评测。完整结论及产物位置见
[真实视频评测报告](REPORT_20260911_REAL_VIDEO.md)。

完成三档 EPIC 参数设置、各三种读出方式、共 117 条诊断回答；suite 比较
盲答、两档 LoRA、显式笔记和直接看帧。12 步及 3 步参数组均答对 3/5，
但原 EPIC 回答重复被复现，调低训练强度后仍有无依据内容。
训练目标质量、事实读回与跨场景关系保持仍未解决。

新增可选训练目标审计、冻结视觉读取对照、严格检查缺题/重复/不同题目的报告工具。
报告将训练损失、事实评分、重复退化与运行成本分别呈现，并纠正了
`n_records=0` 被解释成“没有参数记忆”的通用报告提示。
测试：VideoQA 10 项、桥接 3 项、报告/聚合 44 项通过。

以下“本轮没有访问服务器”等语句属于较早接口实现阶段的历史记录。
